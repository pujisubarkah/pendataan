...
const successToastOptions = {
  style: { background: '#2894D9', color: '#FFFFFF', border: '1px solid #1D78B3' }
}
const errorToastOptions = {
  style: { background: '#111111', color: '#FFFFFF', border: '1px solid #2F2F2F' }
}
...
async function submitProfil() {
  if (!form.instansiId || !form.unit || !form.pic || !form.kontak) {
    toast.error('Mohon lengkapi semua field yang wajib diisi (*)', errorToastOptions)
    return
  }
  ...
  try {
    ...
    toast.error('Terjadi kesalahan saat menyimpan data profil.', errorToastOptions)
  } finally {
